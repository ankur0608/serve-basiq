...
export default function ProductDetail({ params }: { params: { id: string } }) {
  ...
  // Mock Data for Specs since it wasn't in the main interface
  const specs = [
    ...
    <div className="pb-32 bg-white min-h-screen">
      <div className="max-w-6xl mx-auto px-4 py-6 grid md:grid-cols-2 gap-8 md:gap-12">
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 p-8 rounded-3xl border border-gray-100 flex items-center justify-center relative h-[300px] md:h-[500px] group overflow-hidden">
            ...
            <button 
              onClick={() => router.back()} 
              className="absolute top-4 left-4 w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-gray-500 hover:text-slate-900 z-20 transition"
            >
              <FaArrowLeft />
            </button>
            ...
            <button 
              ...
              className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-gray-400 hover:text-red-500 z-20 transition"
            >
             ...
             {[1, 2, 3, 4].map((i) => (
               <div key={i} className="bg-gray-50 border border-gray-100 p-2 rounded-xl h-20 md:h-24 flex items-center justify-center cursor-pointer hover:border-commerce-500 transition">
                 <div className="relative w-full h-full">
                   <Image src={product.img} alt="thumb" fill className="object-contain mix-blend-multiply" />
                 </div>
               </div>
             ))}
          </div>
        ...
        <div className="pt-4">
          ...
          <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100 mb-8">
            ...
            <div>
              ...
              <div className="font-bold text-slate-900 text-lg flex items-center gap-2">
                {product.supplier} <FaCircleCheck className="text-blue-500 text-sm" />
              </div>
            </div>
            <button className="ml-auto text-sm font-bold text-commerce-600 hover:underline">
              View Profile
            </button>
          </div>
          ...
          <div className="flex border-b border-gray-200 mb-6">
            <button 
              ...
              className={clsx("pb-3 mr-8 font-bold text-sm transition border-b-2", activeTab === 'desc' ? "border-slate-900 text-slate-900" : "border-transparent text-gray-400 hover:text-slate-600")}
            >
              Description
            </button>
            <button 
              onClick={() => setActiveTab('specs')}
              className={clsx("pb-3 mr-8 font-bold text-sm transition border-b-2", activeTab === 'specs' ? "border-slate-900 text-slate-900" : "border-transparent text-gray-400 hover:text-slate-600")}
            >
              Specifications
            </button>
            <button 
              onClick={() => setActiveTab('ship')}
              className={clsx("pb-3 font-bold text-sm transition border-b-2", activeTab === 'ship' ? "border-slate-900 text-slate-900" : "border-transparent text-gray-400 hover:text-slate-600")}
            >
              Shipping
            </button>
          </div>
          ...
          <div className="min-h-[150px]">
            {activeTab === 'desc' && (
              <p className="text-gray-600 leading-relaxed whitespace-pre-line animate-fade-in">
                {product.desc}
              </p>
            )}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[70] p-4 md:p-6 flex justify-center pointer-events-none">
        <div className="bg-white/95 backdrop-blur-xl border border-gray-200 shadow-up rounded-2xl p-4 w-full max-w-4xl pointer-events-auto flex items-center gap-3 md:gap-6">
          ...
          <button className="flex-[2] bg-commerce-600 text-white py-3.5 rounded-xl font-bold shadow-lg shadow-commerce-600/30 hover:bg-commerce-700 transition transform active:scale-95 flex items-center justify-center gap-2">
