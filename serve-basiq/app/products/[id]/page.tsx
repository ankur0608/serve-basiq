...
export default async function ProductDetail({ params }: Props) {
  ...
  const product = await prisma.product.findUnique({
    ...
    include: {
      ...
      // ✅ FIX: Fetch ALL Public User Data for the Modal
      user: {
        select: {
          id: true,
          name: true,
          ...
          // Location (City/State only for privacy/context)
          addresses: {
            where: { type: "Work" }, // Prefer Work address
            take: 1,
    ...
    <div className="pb-32 bg-white min-h-screen">
      <div className="max-w-6xl mx-auto px-4 py-6 grid md:grid-cols-2 gap-8 md:gap-12">
        ...
        <div className="pt-4">
          ...
          <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100 mb-8">
            ...
            {/* ✅ MODAL TRIGGER (Pass full user object) */}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-y-3 text-sm">{specs.map((s, i) => (<div key={i} className="contents"><div className="text-gray-500">{s.label}</div><div className="font-medium text-slate-900">{s.value}</div></div>))}</div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[70] p-4 md:p-6 flex justify-center pointer-events-none">
        <div className="bg-white/95 backdrop-blur-xl border border-gray-200 shadow-xl rounded-2xl p-4 w-full max-w-4xl pointer-events-auto flex items-center gap-3 md:gap-6">
          ...
          <ProductWrapper productId={product.id} productName={product.name} productPrice={product.price} productUnit={product.unit} moq={product.moq} currentUser={loggedInUser} userAddresses={loggedInUser?.addresses || []} />
        </div>
      </div>
    </div>
  );
