...
import {
    ...
    FaIndianRupeeSign, FaInstagram, FaFacebook, FaGlobe, FaCheck, FaSpinner
        ...
        <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6">
            <div className="max-w-3xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {/* 3. Socials Card */}
                    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 sm:p-8">
                        <h2 className="font-bold text-lg mb-6 text-slate-900">Social Links (Optional)</h2>
                        <div className="space-y-4">
                            <div className="flex items-center gap-3"><FaInstagram className="text-pink-600 text-xl" /><input name="instagram" onChange={handleInputChange} placeholder="Instagram URL" className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-pink-500 transition" /></div>
                            <div className="flex items-center gap-3"><FaFacebook className="text-blue-600 text-xl" /><input name="facebook" onChange={handleInputChange} placeholder="Facebook URL" className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-blue-600 transition" /></div>
                            <div className="flex items-center gap-3"><FaGlobe className="text-emerald-600 text-xl" /><input name="website" onChange={handleInputChange} placeholder="Website URL" className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-emerald-600 transition" /></div>
                        </div>
                    </div>
...
// import {
//     FaArrowLeft, FaCamera, FaImages, FaLocationDot, FaTag,
//     FaIndianRupeeSign, FaInstagram, FaFacebook, FaGlobe, FaCheck
// } from 'react-icons/fa6';
// import clsx from 'clsx';

// export default function CreateServicePage() {
//     const router = useRouter();
//     const { currentUser, setCurrentUser } = useUIStore(); // Get setCurrentUser to update UI instantly
//     const [loading, setLoading] = useState(false);

//     // Form State matching your Service Schema
//     const [formData, setFormData] = useState({
//         name: '',
//         cat: '',
//         price: '',
...
//     // Handle Image Selection (Converts to Base64 for demo/simple upload)
//     const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, isGallery = false) => {
//         const file = e.target.files?.[0];
//         if (file) {
//             const reader = new FileReader();
//             reader.onloadend = () => {
//                 if (isGallery) setGallery(prev => [...prev, reader.result as string]);
...
//                 loc: formData.loc,
//                 desc: formData.desc,
//                 img: coverImg || "https://via.placeholder.com/400", // Fallback image
//                 gallery: gallery,
//                 social: {
...
//             // ✅ Update Local State: Make user a Worker immediately in the UI
//             if (currentUser) {
//                 setCurrentUser({ ...currentUser, isWorker: true });
//             }
...
//                                     {coverImg ? <img src={coverImg} className="w-full h-full object-cover" /> : <div className="text-center text-slate-400"><FaCamera className="mx-auto mb-2 text-xl" /><span className="text-xs font-bold">Upload Cover</span></div>}
//                                 </div>
//                             </div>
//                             <div>
//                                 <label className="text-xs font-bold text-slate-500 uppercase mb-2 block">Gallery (Add up to 4)</label>
//                                 <div className="grid grid-cols-2 gap-2">
//                                     {gallery.map((img, i) => <img key={i} src={img} className="w-full aspect-square object-cover rounded-lg border border-slate-200" />)}
//                                     {gallery.length < 4 && (
//                                         <div className="relative w-full aspect-square rounded-lg border-2 border-dashed border-slate-300 flex items-center justify-center cursor-pointer hover:bg-blue-50 hover:border-blue-400 transition">
...
//                     <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg shadow-xl hover:bg-black hover:-translate-y-1 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
//                         {loading ? "Creating Profile..." : <><FaCheck /> Create Professional Profile</>}
//                     </button>
//                 </form>
//             </div>
//         </div>
