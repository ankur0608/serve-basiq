...
export default function FavoritesPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            // If we get a 401 or 500, return empty structure to prevent crashes
            if (!res.ok) return { services: [], products: [] };
            return res.json();
        },
    });

    // 2. Remove Favorite Mutation
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500 text-sm mt-1">
                            {activeTab === 'SERVICES' ? "Start exploring services to add them here." : "Browse products and save the best deals."}
                        </p>
                    </div>
                ) : (
