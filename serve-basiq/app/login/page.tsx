...
import { FaGoogle, FaMobileScreen, FaArrowRight, FaSpinner } from 'react-icons/fa6';
...
function LoginForm() {
    ...
    // Use the global store to open the OTP modal
    ...
    const handlePhoneSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // ✅ Open the OTP Modal on top of this page
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#F8F9FC] p-4">
            <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8 border border-slate-100">
                        ...
                        <div className="relative group flex items-center">
                            ...
                            <input
                                ...
                                className={clsx(
                                    ...
                                    phoneError ? "border-red-500 focus:ring-4 focus:ring-red-500/10" : "border-gray-200 focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10",
                                    (isGoogleLoading || isPhoneLoading) && "opacity-50 cursor-not-allowed bg-gray-50"
                                )}
                            />
                        </div>
                        {phoneError && <p className="text-red-500 text-xs font-bold mt-2 ml-1 animate-pulse">{phoneError}</p>}
                    </div>

                    <button type="submit" disabled={isGoogleLoading || isPhoneLoading} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg shadow-lg hover:bg-black transition-all active:scale-95 disabled:opacity-70 flex justify-center items-center gap-2">
                ...
                {/* --- GOOGLE BUTTON --- */}
                <button onClick={handleGoogleLogin} disabled={isGoogleLoading || isPhoneLoading} className="w-full flex items-center justify-center gap-3 py-3.5 border border-gray-200 rounded-xl hover:bg-gray-50 font-bold text-slate-700 transition-all active:scale-95 disabled:opacity-50">
                    {isGoogleLoading ? <FaSpinner className="animate-spin text-red-500" /> : <FaGoogle className="text-xl text-red-500" />}
                    <span>Google</span>
                </button>
...
// ✅ Suspense is required for useSearchParams to work correctly in Next.js builds
