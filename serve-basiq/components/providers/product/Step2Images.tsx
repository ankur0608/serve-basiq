...
interface Step2Props {
    form: ProductForm;
    uploading: boolean;
    activeUploadField: 'productImages' | 'gallery' | null;
...
export function Step2Media({
    ...
    // ✅ Enforce gallery limits: Max 45 images, Max 5 videos
        ...
        for (const file of fileArray) {
            if (file.type.startsWith('image/')) {
                ...
                if (currentImages + addedImages >= 45) {
                    alert("Maximum 45 gallery images allowed. Extra images were skipped.");
                    continue;
                }
                ...
                if (currentVideos + addedVideos >= 5) {
                    alert("Maximum 5 gallery videos allowed. Extra videos were skipped.");
                    continue;
                }
    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-right duration-300">
            ...
            <div>
                <label className={labelClass}>Product Images (Max 5) <span className="text-red-500">*</span></label>
                <div className="grid grid-cols-5 gap-2">
                    {form.productImages.map((url, i) => (
                        <div key={i} className="relative aspect-square rounded-lg overflow-hidden group border border-slate-200">
                            <AppImage src={url} alt={`Product ${i + 1}`} type="thumbnail" className="w-full h-full object-cover" />
                            <button
                                type="button"
                ...
                <p className="text-[10px] text-slate-400 mt-2">Upload up to 5 clear images of your product.</p>
            </div>

            {/* ✅ Updated Gallery Section with visual limits */}
            <div>
                <label className={labelClass}>Gallery (Images: Max 45 | Videos: Max 5)</label>
                <div className="grid grid-cols-4 gap-2">
                    ...
                    {/* Only show upload button if either images or videos can still be added */}
            ...
            <div className="flex gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                    type="submit"
                    disabled={isSaving || form.productImages.length === 0}
