...
export function useServiceForm({ userId, serviceData, userData, userAddress, onComplete, showToast, preSelectedType }: ServiceSettingsProps) {
    ...
    const [form, setForm] = useState(() => ({
        ...
        minDuration: serviceData?.minDuration || 1,
        maxDuration: serviceData?.maxDuration || '',
        advanceNotice: serviceData?.advanceNotice || 0,
        autoConfirm: serviceData?.autoConfirm ?? false,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload = {
                ...
                advanceNotice: Number(form.advanceNotice),
