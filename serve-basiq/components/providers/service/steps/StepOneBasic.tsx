            ...
            <div className="relative">
                <Select
                    ...
                    options={[
                        ...
                        ...activeSubCategories.map((sub: SubCategory) => ({ label: sub.name, value: sub.id }))
                    ]}
                />
            </div>
...
export const StepThreeSchedule = ({ form, handleChange, handleGetLocation, gettingLoc, toggleDay, setStep, DAYS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] }: any) => (
    <div className="space-y-5 animate-in slide-in-from-right duration-300">
        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 shadow-sm">
            <label className="block text-xs font-bold text-slate-700 uppercase mb-4">Service Area</label>
            <div className="space-y-4">
                <div>
                    <Input
                ...
                <button
                    type="button"
                    onClick={handleGetLocation}
                    className={`w-full py-3 rounded-xl flex items-center justify-center gap-2 font-bold text-sm transition border shadow-sm ${form.latitude ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' : 'bg-white text-slate-700 border-slate-200 hover:border-blue-300 hover:text-blue-600'}`}
                >
                    {gettingLoc ? <Loader2 className="animate-spin" size={18} /> : (form.latitude ? <Check size={18} /> : <Navigation size={18} />)}
                    {form.latitude ? "GPS Location Captured" : "Use Current Location"}
                </button>
    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-right duration-300">
            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div>
                    <Input
                        label={listingType === 'RENTAL' ? `RENTAL PRICE (${form.priceType}) (INR)` : `SERVICE PRICE (${form.priceType}) (INR)`}
                        type="number"
                        icon={<BadgeIndianRupee size={20} className="text-slate-400 group-focus-within:text-blue-500" />}
                        placeholder="0.00"
                        value={form.price}
                        onChange={e => handleChange('price', e.target.value)}
