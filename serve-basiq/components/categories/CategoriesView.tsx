...
import { useCategories, Category, SubCategory } from '@/app/hook/useCategories'; // Adjust path as needed
...
export default function CategoriesView() {
    ...
    // --- State ---
    const validTabs = ['ALL', 'SERVICE', 'PRODUCT', 'RENTAL'];
    const isFixedTab = initialTabParam && validTabs.includes(initialTabParam.toUpperCase());

    const defaultTab = isFixedTab
        ? (initialTabParam.toUpperCase() as 'ALL' | 'SERVICE' | 'PRODUCT' | 'RENTAL')
        : 'ALL';

    const [activeTab, setActiveTab] = useState<'ALL' | 'SERVICE' | 'PRODUCT' | 'RENTAL'>(defaultTab);
    const [searchQuery, setSearchQuery] = useState('');
    const [selectedCategory, setSelectedCategory] = useState<Category | null>(null);

    // Sync tab if URL changes
    useEffect(() => {
        if (initialTabParam && validTabs.includes(initialTabParam.toUpperCase())) {
            setActiveTab(initialTabParam.toUpperCase() as any);
        }
    }, [initialTabParam]);
    ...
    // Auto-select category if navigated directly
    useEffect(() => {
                                ...
                                {['ALL', 'SERVICE', 'PRODUCT', 'RENTAL'].map((tab) => (
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            router.replace(`/categories${tab !== 'ALL' ? `?tab=${tab}` : ''}`, { scroll: false });
                                        }}
                                        className={`
