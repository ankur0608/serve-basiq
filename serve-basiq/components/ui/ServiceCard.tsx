      ...
      >
        <div className="relative h-48 w-full bg-gray-100 overflow-hidden cursor-pointer" onClick={handleDetailsClick}>
          <AppImage
            ...
            alt={name}
            type="card"
            className="absolute inset-0 w-full h-full [&_img]:group-hover:scale-105"
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
          />

          <span
            title={categoryName}
            className={`absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm text-white uppercase tracking-wide z-10 max-w-[calc(100%-2.5rem)] truncate block ${
              type === "Rental" ? "bg-orange-500" : "bg-blue-600"
            }`}
          >
            {categoryName}
          </span>
        ...
        <div className="p-3 flex flex-col gap-1 flex-1">
          ...
          <div className="flex gap-2 mt-auto pt-3">
            ...
            <button
              ...
              className="flex-1 bg-black text-white text-xs py-1.5 rounded-lg hover:bg-gray-800 font-medium shadow-sm transition-colors"
            >
