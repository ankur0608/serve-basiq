...
import { FaWallet, FaCalendarCheck, FaBriefcase, FaHeart, FaStar, FaBoxOpen, FaCircleExclamation } from 'react-icons/fa6';
        ...
        <div className="space-y-6">
                ...
                <Link href="/join-pro" className="block relative group cursor-pointer">
                    <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-6 text-white flex items-center justify-between shadow-lg relative overflow-hidden">
                        <div className="relative z-10">
                            ...
                            <p className="text-slate-300 text-sm mb-4">Earn money by offering your skills.</p>
                            <span className="bg-white text-slate-900 px-4 py-2 rounded-lg text-xs font-bold group-hover:bg-gray-100 transition inline-flex items-center gap-2">
                                <FaBriefcase /> Apply Now
                            </span>
                        </div>
                        <FaBriefcase className="absolute -right-6 -bottom-6 text-9xl text-white/5 transform rotate-12 group-hover:rotate-0 transition duration-500" />
                    </div>
                </Link>
            )}
            ...
            {isWorker && !currentUser.isVerified && (
                <div className="bg-orange-50 border border-orange-100 p-4 rounded-2xl flex items-start gap-4 shadow-sm animate-pulse">
                    ...
                    <div>
                        <h3 className="font-bold text-orange-900">Verification Pending</h3>
                        <p className="text-sm text-orange-700 mt-1">Your profile is hidden until approved.</p>
                    </div>
                </div>
            )}
        </div>
...
function StatCard({ icon: Icon, value, label, color }: any) {
    return (
        <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center hover:-translate-y-1 transition duration-300">
            <div className={clsx("w-10 h-10 rounded-full flex items-center justify-center mb-2 text-lg", color)}><Icon /></div>
